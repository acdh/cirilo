<?xml version="1.0"?>
<!-- $Id: bibxml2ris.xsl 450 2008-01-27 20:41:42Z ringler $ -->
 ...
 */
-->
<xsl:stylesheet version="2.0" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:bibtex="http://bibtexml.sf.net/" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:bibfunc="http://bibtexml.sf.net/functions">
  ...
  <xsl:template match="bibtex:journal">
    <xsl:call-template name="field255">
      <xsl:with-param name="risid" select="if(contains(.,'.')) then 'JO' else 'JF'"/>
  ...
  <xsl:template match="bibtex:isbn|bibtex:issn">
    <xsl:call-template name="field">
      ...
      <xsl:with-param name="value" select="concat(local-name(), ':', normalize-space(.))"/>
    </xsl:call-template>
  </xsl:template>
  ...
  <xsl:template match="bibtex:url|bibtex:doi|bibtex:howpublished" mode="url" priority="1">
    <xsl:if test="not(./text() eq '')">
      <xsl:call-template name="field">
        <xsl:with-param name="risid" select="'UR'"/>
        <xsl:with-param name="value" select="if(local-name() eq 'doi') then bibfunc:doi-to-url(text()) else if(local-name() eq 'howpublished') then substring-after(text(),'\url') else text()"/>
      </xsl:call-template>
    </xsl:if>
  </xsl:template>
