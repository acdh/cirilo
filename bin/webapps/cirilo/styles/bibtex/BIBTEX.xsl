<?xml version="1.0"?>
<!-- $Id: bibxml2bib.xsl 437 2008-01-24 15:31:47Z ringler $ -->
<!--
 ...
 */
-->
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:bibtex="http://bibtexml.sf.net/" xmlns:my="bibfunc:bar" xmlns:xs="http://www.w3.org/2001/XMLSchema">
  <xsl:param name="bibtexml.sf.net.encoding" select="'UTF-8'"/>
  <xsl:output method="text" media-type="application/x-bibtex" encoding="UFT-8"/>
  ...
  <xsl:template match="bibtex:author|bibtex:editor" priority="0.6">
    ...
    <xsl:if test="empty(./*) and (. = $brothers[1])">
      <!-- no output for containers -->
      <xsl:text>   </xsl:text>
